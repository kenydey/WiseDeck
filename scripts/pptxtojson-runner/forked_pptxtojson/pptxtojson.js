...
/** WiseDeck fork: expose OOXML p:ph idx/type on elements (upstream does not emit these). */
function wisedeckApplyPlaceholderProps(target, phIdxAttr, phTypeAttr) {
  if (!target || typeof target !== 'object') return target
  const hasIdx = phIdxAttr !== undefined && phIdxAttr !== null && String(phIdxAttr).length > 0
  const hasType = phTypeAttr !== undefined && phTypeAttr !== null && String(phTypeAttr).length > 0
...
async function processCxnSpNode(node, warpObj, source) {
  ...
  return await genShape(node, undefined, undefined, name, type, order, warpObj, source, link, [], phIdxAttr, phTypeAttr)
}

async function genShape(node, slideLayoutSpNode, slideMasterSpNode, name, type, order, warpObj, source, link, groupHierarchy = [], phIdxAttr = undefined, phTypeAttr = undefined) {
