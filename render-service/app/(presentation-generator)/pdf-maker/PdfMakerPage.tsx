...
const PresentationPage = ({ presentation_id }: { presentation_id: string }) => {
  ...
  const fetchUserSlides = async () => {
    try {
      let data;
      if (presentation_id.startsWith("wisedeck-")) {
        const sid = presentation_id.replace(/^wisedeck-/, "");
        const res = await fetch(
          `/api/wisedeck/session?id=${encodeURIComponent(sid)}`
        );
        if (!res.ok) {
          throw new Error(`WiseDeck session fetch failed: ${res.status}`);
        }
        data = await res.json();
      } else {
        data = await DashboardApi.getPresentation(presentation_id);
      }
