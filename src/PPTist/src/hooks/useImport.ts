...
export type ImportPptxFileOptions = {
  cover?: boolean
  fixedViewport?: boolean
  /** When true, returns a Promise that settles when import finishes (embed / WiseDeck). */
  awaitResult?: boolean
}
...
export default () => {
  ...
  const importPPTXFile = (
    ...
    const { cover, fixedViewport, awaitResult } = { ...defaultOptions, ...options }

    const file = files[0]
    if (!file) {
      if (awaitResult) return Promise.reject(new Error('no file'))
      return
    }
