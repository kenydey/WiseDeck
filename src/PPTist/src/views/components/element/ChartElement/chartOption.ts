...
export const getChartOption = ({
  ...
  if (!data?.series?.length || !Array.isArray(data.series[0]) || !data.series[0].length) {
    return null
  }
  const safeThemeColors = Array.isArray(themeColors) && themeColors.length
    ? themeColors
