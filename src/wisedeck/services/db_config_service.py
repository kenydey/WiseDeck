...
class DatabaseConfigService:
    ...
    def __init__(self):
        ...
        self.config_schema = {
            ...
            # WiseDeck official OpenAI-compatible provider (system-level, admin-only for API key and base_url)
            "wisedeck_api_key": {"type": "password", "category": "ai_providers", "admin_only": True},
            "wisedeck_base_url": {"type": "url", "category": "ai_providers", "default": "https://api.openai.com/v1", "admin_only": True},
            "wisedeck_model": {"type": "text", "category": "ai_providers", "default": "MODEL1"},
            
            "default_ai_provider": {"type": "select", "category": "ai_providers", "default": "wisedeck"},
            ...
            async with AsyncSessionLocal() as session:
                ...
                for key, value in config.items():
                    ...
                    if key in self.config_schema:
                        ...
                        # This ensures keys like wisedeck_api_key are accessible to all users
                ...
                try:
                    ...
                    if legacy_value == "gpt-4o" and desired_default:
                        await repo.set_config(
                            ...
                            config_type=self.config_schema["wisedeck_model"]["type"],
                            category=self.config_schema["wisedeck_model"]["category"],
                        )
                        count += 1
                except Exception:
                    pass
...
def _build_user_ai_provider_config(
    ...
    resolved_provider = (provider_name or user_config.get("default_ai_provider") or "openai").strip().lower()
    ...
    provider_configs = {
        ...
        "wisedeck": {
            "api_key": user_config.get("wisedeck_api_key"),
            "base_url": user_config.get("wisedeck_base_url"),
            "model": user_config.get("wisedeck_model"),
            "llm_timeout_seconds": user_config.get("llm_timeout_seconds"),
        },
    }
    provider_configs["gemini"] = dict(provider_configs["google"])

    config = dict(provider_configs.get(resolved_provider, provider_configs["wisedeck"]))
    config["provider_name"] = resolved_provider

    if resolved_provider == "wisedeck":
        logger.info(
            "WiseDeck OpenAI-compatible config - api_key present: %s, base_url: %s, model: %s",
...
async def get_user_role_provider(
    ...
    # (If the user has no default provider configured, fall back to WiseDeck.)
    if not provider_name:
        provider_name = user_config.get("default_ai_provider") or "wisedeck"
