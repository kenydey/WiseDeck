...
def _calculate_safe_parallelism(width: int, height: int, fps: int = 30) -> int:
    ...
    available_mb_env = (os.environ.get("WISEDECK_AVAILABLE_MEMORY_MB") or "").strip()
    if available_mb_env:
        try:
    ...
    env_max = int(os.environ.get("WISEDECK_NARRATION_VIDEO_PARALLELISM", "4") or 4)
...
def _calculate_safe_screenshot_parallelism(width: int, height: int) -> int:
    ...
    env_max = int(os.environ.get("WISEDECK_NARRATION_SCREENSHOT_PARALLELISM", os.environ.get("WISEDECK_NARRATION_VIDEO_PARALLELISM", "4")) or 4)
    cpu_count = os.cpu_count() or 2
    ...
    async def export_project_video(
        ...
        *,
        ...
        if render_mode == "live" and fps == 60:
            allow_60 = str(os.environ.get("WISEDECK_NARRATION_LIVE_ALLOW_60FPS", "")).strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
            ...
            if not allow_60:
                ...
                logger.info("Live narration recording: forcing 30fps (set WISEDECK_NARRATION_LIVE_ALLOW_60FPS=1 to keep 60fps).")
                fps = 30
    ...
    async def _export_project_video_live(
        ...
        *,
        ...
        try:
            ...
            record_mode = (os.environ.get("WISEDECK_NARRATION_VIDEO_RECORD_MODE", "pad") or "pad").strip().lower()
            ...
            full_record_cap_ms = int(os.environ.get("WISEDECK_NARRATION_VIDEO_FULL_RECORD_MAX_MS", "600000") or 600000)
            full_record_cap_ms = max(10_000, full_record_cap_ms)
            record_start_delay_ms = int(
                os.environ.get("WISEDECK_NARRATION_VIDEO_RECORD_START_DELAY_MS", "0") or 0
            )
            record_start_delay_ms = max(0, min(60_000, record_start_delay_ms))

            max_record_ms = int(os.environ.get("WISEDECK_NARRATION_VIDEO_MAX_RECORD_MS", "10000") or 10000)
            max_record_ms = max(600, max_record_ms)
            min_usable_ms = int(os.environ.get("WISEDECK_NARRATION_VIDEO_MIN_USABLE_MS", "1800") or 1800)
            min_usable_ms = max(300, min_usable_ms)
            pad_record_ms_env = (os.environ.get("WISEDECK_NARRATION_VIDEO_PAD_RECORD_MS") or "").strip()
            pad_record_ms = None
            ...
            try:
                record_parallelism = int(os.environ.get("WISEDECK_NARRATION_VIDEO_RECORD_PARALLELISM", "1") or 1)
            except Exception:
            ...
            trans_parallelism = int(
                os.environ.get("WISEDECK_NARRATION_VIDEO_TRANSCODE_PARALLELISM", str(min(2, parallelism))) or 2
            )
            ...
            async def _transcode_one(
                ...
                try:
                    extra_trim_ms = int(os.environ.get("WISEDECK_NARRATION_VIDEO_EXTRA_TRIM_MS", "0") or 0)
                except Exception:
                    extra_trim_ms = 0
                ...
                threads = (os.environ.get("WISEDECK_X264_THREADS") or "2").strip()
                if threads and threads != "0":
                    args.extend(["-threads", threads])
                args.extend(
                    [
                        "-preset",
                        os.environ.get("WISEDECK_X264_PRESET", "ultrafast"),
                        "-crf",
                        os.environ.get("WISEDECK_X264_CRF", "23"),
                        "-pix_fmt",
                        "yuv420p",
                        "-max_muxing_queue_size",
                        os.environ.get("WISEDECK_MAX_MUXING_QUEUE_SIZE", "1024"),
                        "-movflags",
                        "+faststart",
                        out_mp4,
                    ]
    ...
    async def _export_project_video_static(
        ...
        *,
        ...
        try:
            ...
            sem = asyncio.Semaphore(parallelism)
            screenshot_dsf_env = (os.environ.get("WISEDECK_NARRATION_SCREENSHOT_DSF") or "").strip()
            try:
            ...
            for i in rendered_indices:
                ...
                ffmpeg_args = [
                    ...
                    "libx264",
                    "-threads",
                    (os.environ.get("WISEDECK_X264_THREADS") or "2"),
                    "-preset",
                    os.environ.get("WISEDECK_X264_PRESET", "ultrafast"),
                    "-crf",
                    os.environ.get("WISEDECK_X264_CRF", "23"),
