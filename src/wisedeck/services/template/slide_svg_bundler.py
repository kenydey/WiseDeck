...
def bundle_slide_svgs(
    ...
    """
    Returns (svg_template, html_template) with scrollable HTML wrapper.
    """
    if not svg_paths:
    ...
    return svg_template, wrap_svg_vertical_stack_html(svg_template, "Imported Template")
...
def bundle_workspace_svgs(svg_dir: Path, bundle_mode: BundleMode) -> Tuple[str, str, List[str]]:
    """
    Reads slide_*.svg from workspace svg_dir.
    Returns (svg_template, html_template, warnings).
    """
    warnings: List[str] = []
    ...
    svg_t, html_t = bundle_slide_svgs(paths, bundle_mode=bundle_mode)
    return svg_t, html_t, warnings
