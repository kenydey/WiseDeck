...
class GlobalMasterTemplateService:
    ...
    async def _get_template_role_provider_async(self):
        """Get provider and settings for template generation role (async version with WiseDeck support)"""
        ...
        if self.user_id is not None:
            try:
                ...
                provider_name = self.provider_name or user_config.get(role_provider_key) or user_config.get("default_ai_provider") or "wisedeck"
                ...
                # Use get_user_ai_provider which properly handles WiseDeck system-level config
    ...
    async def generate_template_with_ai(self, prompt: str, template_name: str, description: str = "",
        ...
        if template_workspace_id:
            ...
            try:
                ...
                ws_full_cached = json.loads(ws_manifest_path.read_text(encoding="utf-8"))
                pl_raw = ws_full_cached.get("pptx_layout")
                ...
                pr_sum = ws_full_cached.get("pptx_readable_summary")
                if isinstance(pr_sum, dict) and pr_sum:
                    readable_extra = (
                        "\n【pptx_readable 摘要】主题色、占位标记并集、元素类型计数（pptxtojson 解析）\n"
                        + json.dumps(pr_sum, ensure_ascii=False, separators=(",", ":"))[:7000]
                        + "\n"
                    )
            except Exception as ws_err:
                logger.warning("Failed to load workspace manifest extras (layout/readable summary): %s", ws_err)
            ...
            if readable_extra:
                workspace_manifest_hint += readable_extra
