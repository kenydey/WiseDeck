...
class PollinationsProvider(ImageGenerationProvider):
    ...
    async def generate(self, request: ImageGenerationRequest) -> ImageOperationResult:
        ...
        try:
            ...
            # 发送请求
            async with http_get(api_url, headers=headers, timeout=build_timeout(self.timeout)) as response:
                if response.status_code == 200:
                    ...
                    return ImageOperationResult(
                        success=False,
                        message=f"API request failed with status {response.status_code}: {error_text}"
                    )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with http_get(test_url, headers=headers, timeout=build_timeout(10.0)) as response:
