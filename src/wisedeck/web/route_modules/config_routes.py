...
@router.get("/")
async def web_root():
    """Anonymous/local mode: root goes to dashboard."""
    from fastapi.responses import RedirectResponse
...
async def web_ai_config(
    ...
    def _is_provider_configured(provider: str) -> bool:
        ...
        if name == "wisedeck":
            # WiseDeck credentials are system-scoped; non-admin users can still use it if configured.
            return _has_value(system_config.get("wisedeck_api_key")) and _has_value(system_config.get("wisedeck_base_url"))

        return False

    provider_status = {
        provider: _is_provider_configured(provider)
        for provider in ["wisedeck", "openai", "anthropic", "google", "ollama"]
    }
...
async def get_ai_providers_config(
    ...
    config: Dict[str, Any] = {
        ...
        "wisedeck_model": current_config.get("wisedeck_model", ""),
    }
    if user.is_admin:
        config["wisedeck_api_key"] = current_config.get("wisedeck_api_key", "")
        config["wisedeck_base_url"] = current_config.get("wisedeck_base_url", "")
...
@router.get("/api/ai/providers/wisedeck/models")
@router.get("/api/ai/providers/landppt/models", include_in_schema=False)
async def get_wisedeck_models(
    request: Request,
    user: User = Depends(get_current_user_required)
):
...
async def test_provider_connection(
    ...
    try:
        ...
        # WiseDeck 使用系统级配置，其余提供商使用用户配置。
        if provider == 'wisedeck':
        ...
        if not base_url:
            default_urls = {
                ...
                'wisedeck': 'https://api.openai.com/v1',
            }
