...
async def export_project_pdf_async(
    ...
    Returns immediately with task_id. Poll /api/wisedeck/tasks/{task_id} for progress.
    Download result from /api/wisedeck/tasks/{task_id}/download when completed.
    """
    try:
        project = await ppt_service.project_manager.get_project(project_id, user_id=user.id)
        if not project:
    ...
    if task.status.value == "completed" and task.result:
        ...
        if isinstance(task.result, dict):
            if task.task_type in (
                ...
                response["download_url"] = f"/api/wisedeck/tasks/{task_id}/download"
