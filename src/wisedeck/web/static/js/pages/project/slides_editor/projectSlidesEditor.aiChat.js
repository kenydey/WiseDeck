...
async function saveSlideOutline() {
    ...
    try {
        // 保存大纲到数据库
        const response = await fetch(`/projects/${window.wisedeckEditorConfig.projectId}/update-outline`, {
            method: 'POST',
            headers: {
...
async function getSelectedGlobalTemplate() {
    try {
        const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/selected-global-template`);
        if (!response.ok) {
            return null;
        }
        const data = await response.json();
