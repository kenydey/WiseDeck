        ...
        async function exportSpeechScript(format) {
            ...
            try {
                const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/speech-script/export`, {
                    method: 'POST',
                    headers: {
        ...
        async function refreshSpeechScriptsDialogData() {
            ...
            const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/speech-scripts?language=${encodeURIComponent(lang)}`, {
        ...
        function startSpeechHumanizeProgressTracking(taskId, progressToast, options = {}) {
            ...
            const interval = setInterval(async () => {
                ...
                try {
                    const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/speech-scripts/progress/${taskId}`, {
                        credentials: 'same-origin'
                    });
                    const result = await response.json().catch(() => ({}));
        ...
        async function humanizeSingleSpeechScript(slideIndex, scriptIndex, triggerButton) {
            ...
            try {
                const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/speech-scripts/humanize`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        async function saveSpeechScript(slideIndex, scriptIndex) {
            ...
            try {
                ...
                // 调用API保存
                const lang = getSpeechLanguage();
                const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/speech-scripts/slide/${slideIndex}?language=${encodeURIComponent(lang)}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        async function regenerateSingleScript(slideIndex) {
            ...
            try {
                ...
                // 调用生成API
                const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/speech-script/generate`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
