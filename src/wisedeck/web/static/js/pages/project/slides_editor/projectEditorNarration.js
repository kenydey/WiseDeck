...
function getNarrationLanguage() {
        ...
        async function uploadNarrationReferenceAudio(file) {
            ...
            const resp = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/narration/reference-audio`, {
                method: 'POST',
        ...
        async function generateNarrationAudio() {
            ...
            try {
                ...
                const resp = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/narration/generate`, {
                    method: 'POST',
                    credentials: 'same-origin',
        ...
        async function playNarrationForSlide(slideIndex) {
            ...
            const url = `/api/projects/${window.wisedeckEditorConfig.projectId}/narration/audio/${slideIndex}?language=${encodeURIComponent(lang)}`;
            try {
        ...
        function openNarrationPresentation() {
            const lang = getNarrationLanguage();
            const url = `/projects/${window.wisedeckEditorConfig.projectId}/fullscreen?narration=1&language=${encodeURIComponent(lang)}`;
        ...
        async function exportNarrationVideo() {
            try {
                ...
                const resp = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/export/narration-video`, {
                    method: 'POST',
                    credentials: 'same-origin',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                closeProgressToast(progressToast);
                const downloadUrl = taskData.download_url || `/api/wisedeck/tasks/${taskId}/download`;
        ...
        async function exportNarrationAudio() {
            ...
            try {
                ...
                const resp = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/export/narration-audio`, {
                    method: 'POST',
                    credentials: 'same-origin',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
