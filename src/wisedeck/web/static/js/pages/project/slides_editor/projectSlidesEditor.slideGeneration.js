...
async function pollBackgroundTaskUntilDone(taskId, { timeoutMs = 30 * 60 * 1000, intervalMs = 2000, onTick } = {}) {
    ...
    while (true) {
        const resp = await fetch(`/api/wisedeck/tasks/${taskId}`);
        if (!resp.ok) {
...
function regenerateSlideByIndex(slideIndex) {
    ...
    if (confirm(`确定要重新生成"${slideTitle}"吗？这将覆盖现有内容。`)) {
        ...
        (async () => {
            try {
                const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/slides/${slideIndex + 1}/regenerate/async`, {
                    method: 'POST',
...
async function batchRegenerateSlides({ slideIndices, regenerateAll }) {
    ...
    try {
        ...
        const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/slides/batch-regenerate/async`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function cleanupExcessSlides() {
    try {
        ...
        const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/slides/cleanup`, {
            method: 'POST',
            headers: {
...
async function saveSingleSlideToServer(slideIndex, htmlContent, options = {}) {
    try {
        ...
        const response = await fetch(`/api/projects/${window.wisedeckEditorConfig.projectId}/slides/${slideIndex}/save`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
