...
@router.get("/api/config/wisedeck/models")
@router.get("/api/config/landppt/models", include_in_schema=False)
async def get_wisedeck_models(
    ...
    try:
        ...
        logger.error(f"Failed to get WiseDeck models: {e}")
        return {
            "success": False,
...
@router.post("/api/config/wisedeck/test")
@router.post("/api/config/landppt/test", include_in_schema=False)
async def test_wisedeck_provider(
    user: User = Depends(get_current_user_required)
):
    """
    ...
    try:
        ...
        logger.error(f"Failed to test WiseDeck provider: {e}")
        return {
            "success": False,
            "error": f"测试失败: {str(e)}"
        }
...
async def get_system_env_file(user: User = Depends(get_current_admin_user)):
