...
class AppConfig(BaseSettings):
    ...
    database_url: str = Field(default="sqlite:///./wisedeck.db", env="DATABASE_URL")
    auto_migrate_on_startup: bool = Field(default=True, env="WISEDECK_AUTO_MIGRATE_ON_STARTUP")
    auto_migrate_fail_fast: bool = Field(default=True, env="WISEDECK_AUTO_MIGRATE_FAIL_FAST")
    auto_migrate_lock_timeout_seconds: int = Field(default=300, env="WISEDECK_AUTO_MIGRATE_LOCK_TIMEOUT_SECONDS")
    auto_migrate_lock_stale_seconds: int = Field(default=900, env="WISEDECK_AUTO_MIGRATE_LOCK_STALE_SECONDS")

    
    # Security Configuration
    secret_key: str = Field(default="your-secret-key-here", env="SECRET_KEY")
    access_token_expire_minutes: int = Field(default=20160, env="ACCESS_TOKEN_EXPIRE_MINUTES")  # 2 weeks
    enable_api_docs: bool = Field(default=True, env="WISEDECK_ENABLE_API_DOCS")
    bootstrap_admin_enabled: bool = Field(default=False, env="WISEDECK_BOOTSTRAP_ADMIN_ENABLED")
    bootstrap_admin_username: Optional[str] = Field(default=None, env="WISEDECK_BOOTSTRAP_ADMIN_USERNAME")
    bootstrap_admin_password: Optional[str] = Field(default=None, env="WISEDECK_BOOTSTRAP_ADMIN_PASSWORD")

    # Machine-to-machine API authentication (for n8n / automation)
    # Single key mode: WISEDECK_API_KEY + WISEDECK_API_KEY_USER
    # Multi key mode: WISEDECK_API_KEYS="user1:key1,user2:key2" (also supports user=key)
    api_key: Optional[str] = Field(default=None, env="WISEDECK_API_KEY")
    ...
    smtp_from_name: str = Field(default="WiseDeck", env="SMTP_FROM_NAME")
    ...
    resend_from_name: str = Field(default="WiseDeck", env="RESEND_FROM_NAME")
    ...
    def get_api_key_bindings(self) -> List[Tuple[str, str]]:
        ...
        Supports:
        - WISEDECK_API_KEY + WISEDECK_API_KEY_USER
        - WISEDECK_API_KEYS="user1:key1,user2:key2" (also accepts user=key)
        """
