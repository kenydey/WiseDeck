...
class DatabaseMigration:
    ...
    def _register_migrations(self):
        ...
        # Migration 013: Rename DB config keys landppt_* -> wisedeck_* and provider id landppt -> wisedeck
        self.migrations.append({
            "version": "013",
            "name": "rename_landppt_config_keys_to_wisedeck",
            "description": "Rename user_configs keys landppt_api_key/base_url/model to wisedeck_*; normalize provider values",
    ...
    async def _migration_013_up(self, session: AsyncSession):
        """Copy landppt_* rows to wisedeck_* when missing, delete legacy keys, normalize provider id values."""
        logger.info("Applying migration 013: landppt_* config keys -> wisedeck_*")
        try:
            if not await self._table_exists(session, "user_configs"):
                logger.info("user_configs table not found; skipping migration 013")
                return

            now = time.time()
            pairs = (
                ("landppt_api_key", "wisedeck_api_key"),
                ("landppt_base_url", "wisedeck_base_url"),
                ("landppt_model", "wisedeck_model"),
            )
                ...
                await session.execute(
                    text(
                        ...
                        INSERT INTO user_configs (
                            user_id, config_key, config_value, config_type, category, created_at, updated_at
                        )
                        SELECT
                            o.user_id,
                            :new_key,
                            ...
                            o.created_at,
                            :now
                        FROM user_configs AS o
                        WHERE o.config_key = :old_key
                          AND NOT EXISTS (
                            SELECT 1
                            FROM user_configs AS n
                            WHERE n.config_key = :new_key
                              AND (
                                (n.user_id = o.user_id)
                                OR (n.user_id IS NULL AND o.user_id IS NULL)
                              )
                          )
                        """
                    ),
                    {"old_key": old_key, "new_key": new_key, "now": now},
                )
            ...
            await session.execute(
                text(
                    ...
                    WHERE config_key IN ('landppt_api_key', 'landppt_base_url', 'landppt_model')
                    """
                )
            )
            ...
            provider_keys = (
                ...
                "speech_script_model_provider",
                "vision_analysis_model_provider",
            ...
            for pk in provider_keys:
                await session.execute(
                    text(
                        """
                        UPDATE user_configs
                        SET config_value = 'wisedeck',
                            updated_at = :now
                        WHERE LOWER(TRIM(config_value)) = 'landppt'
                          AND config_key = :pk
                        """
                    ),
                    {"now": now, "pk": pk},
    ...
    async def _migration_013_down(self, session: AsyncSession):
        """Best-effort rollback: wisedeck_* provider keys -> landppt_*; provider id wisedeck -> landppt."""
        logger.info("Rolling back migration 013 (best-effort)")
        try:
            if not await self._table_exists(session, "user_configs"):
                return
            ...
            now = time.time()
            pairs = (
                ("wisedeck_api_key", "landppt_api_key"),
                ("wisedeck_base_url", "landppt_base_url"),
                ("wisedeck_model", "landppt_model"),
            )
            for old_key, new_key in pairs:
                await session.execute(
                    text(
            ...
            await session.execute(
                text(
                    """
                    DELETE FROM user_configs
                    WHERE config_key IN ('wisedeck_api_key', 'wisedeck_base_url', 'wisedeck_model')
                    """
                )
            )
            ...
            for pk in provider_keys:
                await session.execute(
                    text(
                        """
                        UPDATE user_configs
                        SET config_value = 'landppt',
                            updated_at = :now
                        WHERE LOWER(TRIM(config_value)) = 'wisedeck'
                          AND config_key = :pk
                        """
                    ),
                    {"now": now, "pk": pk},
