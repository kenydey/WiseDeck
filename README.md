...
- **参考 PPT/PPTX 提取（无 Office COM）**：服务端使用 LibreOffice 将演示文稿渲染为 PDF，再由 PyMuPDF 导出多页 PNG/SVG；同时用 python-pptx 抽取结构化版式信息，用于 AI 母版生成时的多参考页上下文
...
- Python 3.11+
- Git
- SQLite 3（随 Python/系统提供即可）
- **ffmpeg**（讲解视频导出等能力需要；Windows 可用 `winget install FFmpeg`，Linux 用发行版包管理器如 `apt install ffmpeg`）
- **Playwright Chromium**（PDF/导出相关路径；本机安装依赖后执行 `uv run playwright install chromium`）
- Docker + Docker Compose（可选，见下文「Docker 部署」）
...
### 本地运行详细步骤（Windows / Linux）

以下与当前仓库的 `docker-compose*.yaml`、`run.py` 及环境变量命名（`WISEDECK_*`）一致。

#### 1. 准备环境（Windows PowerShell）

1. 安装 **Python 3.11+**（安装时勾选加入 PATH，或使用 `py -3.11`）。
2. 安装 **uv**：`powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"`（与下方「方法一」相同）。
3. 安装 **ffmpeg**（见系统要求）。
4. 在仓库根目录执行 `uv sync --extra dev`。
5. 复制环境文件：`Copy-Item .env.example .env`，编辑 `.env`：至少设置 **`SECRET_KEY`** 与至少一种 **AI Key**（如 `OPENAI_API_KEY`）；可选设置 **`WISEDECK_BOOTSTRAP_ADMIN_ENABLED=true`** 及 **`WISEDECK_BOOTSTRAP_ADMIN_USERNAME` / `WISEDECK_BOOTSTRAP_ADMIN_PASSWORD`** 以首次启动创建管理员。
6. （按需）`uv run playwright install chromium`。
7. 启动：`uv run python run.py`。浏览器访问 **http://localhost:8000**；API 文档 **http://localhost:8000/docs**；健康检查 **http://localhost:8000/health**。

**IDE 调试（Cursor / VS Code）**：将工作目录设为仓库根目录，保证 **`PYTHONPATH` 包含 `src`**（与 `run.py` 行为一致）。可用模块方式启动 Uvicorn，例如参数 `wisedeck.main:app --reload --host 0.0.0.0 --port 8000`。若使用 `reload` 子进程，断点调试时如遇异常可暂时将 `.env` 中 `RELOAD=false` 或单进程运行。

#### 2. 准备环境（Linux bash）

1. 安装 **Python 3.11+**、`git`、`ffmpeg`（如 Debian/Ubuntu：`sudo apt update && sudo apt install -y ffmpeg git`）。
2. 安装 **uv**：`curl -LsSf https://astral.sh/uv/install.sh | sh`（按提示将 `uv` 加入 PATH）。
3. `cd` 到克隆后的仓库根目录，`uv sync --extra dev`。
4. `cp .env.example .env` 并按上节同样原则编辑。
...
- **生产编排** [`docker-compose.yml`](docker-compose.yml)：应用映射 **`8000:8000`**；PostgreSQL 映射 **`6004:5432`**（本机用 DBeaver/psql 连接时用 `localhost:6004`）。启动：`docker compose up -d --build`；看日志：`docker compose logs -f wisedeck`（服务名为 compose 中的 `wisedeck`）。
- **开发编排** [`docker-compose-dev.yaml`](docker-compose-dev.yaml)：挂载当前目录到容器、默认 `RELOAD=true`。启动：`docker compose -f docker-compose-dev.yaml up -d --build`；日志：`docker compose -f docker-compose-dev.yaml logs -f wisedeck`。开发容器名为 **`wisedeck-dev`**，可用 `docker exec -it wisedeck-dev bash` 进入排查。

均需事先准备根目录 `.env`（含 `SECRET_KEY`、`POSTGRES_*` 与 compose 内数据库 URL 一致、AI Key 等）。

#### 4. 结构化 PPTX（内置）

结构化导出 **`GET .../export/structured-pptx`** 由 WiseDeck 主进程完成：默认 **`mode=auto`** 使用 Playwright 与编辑器同源的 **slides-html**（测量源见 **`WISEDECK_STRUCTURED_PPTX_MEASUREMENT_SOURCE`**，默认 `homomorphic-editable`）。可选 **`mode=homomorphic_editable`** 在同一路径下运行服务端 dom-to-pptx 并可合并原生图表。详见 [STRUCTURED_EXPORT_VENDOR.md](STRUCTURED_EXPORT_VENDOR.md)。无需单独的 Presenton Next.js 服务。
...
默认不会自动创建管理员账号；如需初始化管理员，请显式配置 `WISEDECK_BOOTSTRAP_ADMIN_ENABLED=true`、`WISEDECK_BOOTSTRAP_ADMIN_USERNAME` 和 `WISEDECK_BOOTSTRAP_ADMIN_PASSWORD`。
...
主要配置项（常用项见 `.env.example`，高级项可参考 `src/wisedeck/core/config.py`）：
...
1. 先使用你的管理员账号登录一次获取 `session_id`（管理员可通过 `WISEDECK_BOOTSTRAP_ADMIN_*` 环境变量显式初始化，或在 Web 界面中自行创建）：
...
- `WISEDECK_ALLOW_HEADER_SESSION_AUTH` 默认关闭；只有显式设置为 `true` 时，才支持通过 `X-Session-Id` 头在非浏览器客户端传递会话。
- 全局 Key 适合服务到服务调用；用户自定义 Key 更适合多租户或个人自动化流程。
- `WISEDECK_ENABLE_API_DOCS=true` 时可访问 `/docs`、`/redoc` 和 `/openapi.json`；默认值已开启。
