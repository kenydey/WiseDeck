...
#### 4. 可选：结构化 PPTX 渲染服务（render-service）

高保真 **structured-pptx** 等导出需单独启动 Next 渲染服务，并在主应用环境变量中设置 **`WISEDECK_RENDER_SERVICE_URL`**（例如 `http://127.0.0.1:5000`）。步骤见 [render-service/README_WISEDECK.md](render-service/README_WISEDECK.md)。
