...
def test_get_system_config_will_initialize_system_defaults(monkeypatch):
    import wisedeck.api.config_api as config_api
    ...
    assert result["config"]["default_ai_provider"] == "wisedeck"
    assert result["config"]["wisedeck_model"] == "MODEL1"
